...
void main() 
{
	...
	vec3 worldpos = inWPos;
	
	// Diffuse color
	vec3 diffuse = texture ( sampler2D ( diffuseTexture, textureSampler ), inTex).rgb;
	
	// Normals
	vec3 normal = normalize(inWNormal);
	vec3 tangent = normalize(inWTan);
	...
	vec4 indir = vec4(0.0);
	{
		...
		const float conesPerFragment = 15;
	...
	// show the AO
	//acout += vec3(indir.a,indir.a,indir.a);
	// show the specular
	//acout += vec3(specular.xyz);
	// show the color bleeding/indirect diffuse

	outColor = (vec4(specular.xyz + indir.xyz + diffuse,1)) * indir.a* indir.a;
}